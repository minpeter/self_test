...
def searchSchool(schoolname, loginType, lctnScCode, schulCrseScCode):
    ...
    paramDict = {
        'lctnScCode' : lctnScCode,    #교육청코드 01(서울), 02(부산), 03(대구), 04(인천), 05(광주), 06(대전), 07(울산), 08(세종), 10(경기도), 11(강원), 12(충청북), 13(충청남), 14(전라북), 15(전라남), 16(경상북), 17(경상남), 18(제주) (로그인타입이 학교일떄만)
        'schulCrseScCode' : schulCrseScCode,    #학교종류코드 1(유치원), 2(초등학교), 3(중학교), 4(고등학교), 5(특수학교) (로그인타입이 학교일떄만)
